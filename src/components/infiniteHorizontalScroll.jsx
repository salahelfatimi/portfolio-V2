...
export default function EmblaCarousel() {
  ...
  return (
    <div className="relative overflow-hidden  py-4  select-none  active:cursor-grabbing cursor-grab bg-[#fe3c01] pr-10" ref={emblaRef}>
          <div className="flex flex-row gap-10  items-center   ">
            {keywords.map((ele, index) => (
          <h2 className="  text-black  even:text-white text-2xl whitespace-nowrap break-keep font-black  " key={index}><span>{ele}</span></h2>
      ))}
      </div>
    </div>
  );
