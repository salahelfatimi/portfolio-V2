...
export default function WhatIKnow(){
    ...
    return(
        ...
        <div id="wath-i-know" className=" container  relative min-h-screen flex flex-col justify-start items-center " >
            ...
            <div className="absolute animate-pulse bottom-0 left-1/2 transform -translate-x-1/2 translate-y-0 w-[400px] lg:w-[1000px] h-[800px] bg-[#fe3c01]/50 opacity-70 rounded-full blur-3xl z-0"></div>
            ...
            <div className="flex relative z-50  flex-wrap columns-6 justify-center gap-4 mt-6">
                {codingSkills.map((skill, i) => (
                    <span key={i} className={` ${ i < codingSkills.length / 2 ? 'shadow-[0_0_40px_rgba(147,235,255,0.5)] border-primary' : 'border-[#fe3c01] shadow-[0_0_40px_rgba(254,60,1,0.5)]'} text-white px-8 py-4 text-4xl rounded-lg border bg-bg `}>
                        {skill}
                    </span>
                ))}
            </div>
